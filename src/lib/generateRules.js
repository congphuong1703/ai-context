import { LANGUAGES, IDES, CONVENTIONS, FRAMEWORKS } from "@/data";
...
export function generateOutput(config) {
  ...
  const framework = config.framework ?? DEFAULTS.framework;
  const ide = config.ide ?? DEFAULTS.ide;
  const convention = config.convention ?? DEFAULTS.convention;
  const eslintRequired = config.eslintRequired ?? DEFAULTS.eslintRequired;
  const prettierRequired = config.prettierRequired ?? DEFAULTS.prettierRequired;

  const L = LANGUAGES.find((x) => x.id === language)?.label || language;
  const IDE = IDES.find((x) => x.id === ide)?.label || ide;
  const list = FRAMEWORKS[language];
  const F = framework && list ? list.find((x) => x.id === framework)?.label : null;
  const stack = F ? `${L} + ${F}` : L;
  const filename = IDES.find((x) => x.id === ide)?.file || ".cursorrules";
...
## 🎯 ROLE & EXPERTISE
You are a senior ${L} engineer.
You write clean, production-ready, maintainable code.
You think step-by-step and plan before implementing.
You ask for clarification when requirements are ambiguous.
You follow the rules below precisely and without exception.

## 🧱 ARCHITECTURE
- Strict separation of concerns: presentation / business logic / data access
- Dependency injection over direct instantiation
- Single Responsibility: one purpose per file, class, function
...
- Max file length: 300 lines — extract when approaching limit
- Define explicit public API surfaces for all modules

## 📐 CODE QUALITY
- Descriptive names: verbs for functions, nouns for variables/types
- No magic numbers or strings — named constants only
- All public APIs documented: JSDoc / Javadoc / docstring + example
- Functions: max 40 lines, max 4 params (use DTOs/objects beyond that)
- Prefer composition over inheritance
- Remove dead code immediately — never comment out and leave
...
- Typed errors with codes — avoid generic new Error("something failed")
